...
%       - speed scheduling: f(vx) = min(vx/v_ref, 2)
   ...
   if ~isfield(ctrlState, 'filteredSteer'), ctrlState.filteredSteer = 0; end
    
    error_yaw = yawRateRef - yawRate;
    
    % [핵심] 0.5 게인으로 올리되, 이전 값과 8:2로 섞는 필터 적용 (매틀랩 튕김 방지)
    steer_target = 0.5 * error_yaw; 
    ctrlState.filteredSteer = 0.8 * ctrlState.filteredSteer + 0.2 * steer_target;
    
    deltaAdd.steerAngle = max(min(ctrlState.filteredSteer, LIM.MAX_STEER_ANGLE), -LIM.MAX_STEER_ANGLE);
